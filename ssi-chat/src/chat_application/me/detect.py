...
_CAN_I_CREATE = re.compile(
    ...
    r"may i create|"
    r"(permission|allowed) to create (a )?payment"
    r")\b",
    re.IGNORECASE,
)
...
_WHO_CAN_CREATE = re.compile(
    ...
    r"list .{0,40}(who can create|payment creators?)|"
    r"who (can|may) draft"
    r")\b",
    re.IGNORECASE,
)
...
def detect_me_intent(message: str) -> MeIntent | None:
    ...
    if _WHO_CAN_CREATE.search(text) and re.search(r"\bpayment", text, re.IGNORECASE):
        return MeIntent(
            kind="who_can_create",
            action="CREATE",
            entity_type="payment",
