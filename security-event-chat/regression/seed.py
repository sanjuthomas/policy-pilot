...
def _post_action(client: httpx.Client, harness_url: str, step: SeedStep) -> dict[str, Any]:
    base = harness_url.rstrip("/")
    action = step.action

    if action == "run-policy-scenario":
        response = client.post(f"{base}/api/actions/run-policy-scenario", timeout=300.0)
    elif action == "run-payment-policy-scenario":
        response = client.post(f"{base}/api/actions/run-payment-policy-scenario", timeout=300.0)
    elif action in {
        "create-instructions",
        "submit-instructions",
...
def run_seed(harness_url: str, config: SeedConfig) -> None:
    with httpx.Client(timeout=60.0) as client:
        for step in config.steps:
            _post_action(client, harness_url, step)


def fetch_harness_status(harness_url: str) -> dict[str, Any]:
    with httpx.Client(timeout=30.0) as client:
        response = client.get(f"{harness_url.rstrip('/')}/api/status")
        response.raise_for_status()
