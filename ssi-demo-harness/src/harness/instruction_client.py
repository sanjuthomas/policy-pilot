...
class InstructionServiceClient:
    ...
    def list_instructions(
        ...
        *,
        ...
        if owning_lob:
            params["owning_lob"] = owning_lob
        url = self._url("/instructions")
        with httpx.Client(timeout=30.0) as client:
            return client.get(
                url,
                headers=obo_headers(session),
    ...
    def suspend_instruction(
        ...
        return self.request(
            "POST",
            f"/instructions/{instruction_id}/suspend",
            session=session,
        )

    def reactivate_instruction(
        self, session: SessionCredentials, instruction_id: str
    ) -> httpx.Response:
        return self.request(
            "POST",
            f"/instructions/{instruction_id}/reactivate",
            session=session,
        )
