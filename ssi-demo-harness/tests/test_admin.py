...
def test_admin_login_success() -> None:
    ...
    with patch("harness.auth_routes.settings.zitadel_service_pat", "pat"), patch(
        "harness.auth_routes.settings.oidc_issuer_url",
        "http://localhost:8080",
    ), patch("harness.auth_routes.ZitadelLoginClient") as mock_client:
