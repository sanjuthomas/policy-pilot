...
def test_stats_and_components_endpoints(client):
    test_client, _, _, _ = client
    status = {
    ...
    with patch("etl.main.component_status", AsyncMock(return_value=status)):
        ...
        components = test_client.get("/api/components").json()
    assert stats["all_ok"] is False
    assert components["kafka"]["ok"] is False
...
def test_cypher_generate_success(client):
    ...
    mock_response.json.return_value = {"cypher": "MATCH (n) RETURN n LIMIT 5"}
    ...
    with patch("etl.main.httpx.AsyncClient", return_value=mock_http):
        response = test_client.post(
            "/api/cypher/generate",
...
def test_cypher_generate_unreachable(client):
    test_client, _, _, _ = client
    mock_http = AsyncMock()
    mock_http.__aenter__ = AsyncMock(return_value=mock_http)
    mock_http.__aexit__ = AsyncMock(return_value=False)
