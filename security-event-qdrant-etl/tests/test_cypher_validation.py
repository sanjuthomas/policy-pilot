...
import pytest
from etl.main import _validate_read_only_cypher


def test_valid_read_query():
    _validate_read_only_cypher("MATCH (n) RETURN n LIMIT 10")
...
def test_missing_limit_raises():
    with pytest.raises(ValueError, match="LIMIT clause"):
        _validate_read_only_cypher("MATCH (n) RETURN n")


def test_optional_match_allowed():
    _validate_read_only_cypher("OPTIONAL MATCH (n)-[:REL]->(m) RETURN n, m LIMIT 20")


def test_unwind_allowed():
    _validate_read_only_cypher("UNWIND [1,2,3] AS x RETURN x LIMIT 3")
