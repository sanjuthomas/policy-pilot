...
**Skills** (mutation — use **Payments** mode)

- _Can you create a payment for instruction ID 20260705-FICC-I-31? Value date tomorrow; amount: 12 million USD._ **`skill`** (sign in as `pay-101` / `pay-205`)
...
- _Please submit payment 20260715-FICC-P-9 for approval._ **`skill`** (sign in as `fo-ficc-101`)
...
- _Please cancel payment 20260715-FICC-P-9._ **`skill`** (sign in as `pay-101` / `pay-205`)

  Scripted **cancel-payment** skill: load DRAFT or SUBMITTED payment → OPA `CANCEL` preflight (requires `PAYMENT_CREATOR` + `MIDDLE_OFFICE`) → confirmation card → cancel only on **Go**. Full write-up: **[Cancel-payment skill](docs/cancel-payment-skill.md)**.
...
| **[Cancel-payment skill](docs/cancel-payment-skill.md)** | Middle-office creator cancels DRAFT or SUBMITTED payment: OPA `CANCEL` preflight, same confirmation card, cancel on **Go**. |
...
| **[Sample questions](docs/sample-questions.md)** | Curated demo questions by retrieval path (`graph`, `tools`, `skill`, `vector`), including Policies-mode and create/submit/approve/cancel payment skills. |
...
| **Skills** | Scripted multi-step actions (create / submit / approve / cancel payment) | Mutate only after OPA preflight + explicit Go / No Go |
...
Payment creators (`pay-*`), desk submitters (`fo-*`), and funding approvers (`pay-*` / `mo-*`) use the same route → retrieve → synthesize path for questions, and can also run **mutation skills** (create-payment, submit-payment, approve-payment, cancel-payment) after OPA preflight and an explicit **Go / No Go**.
...
Implementation entry point: `RagService.ask()` delegates to `RagPipelineOrchestrator` in `ssi-chat/src/chat_application/pipeline/orchestrator.py`. Mutation skills: **[create-payment](docs/create-payment-skill.md)**, **[submit-payment](docs/submit-payment-skill.md)**, **[approve-payment](docs/approve-payment-skill.md)**, **[cancel-payment](docs/cancel-payment-skill.md)** (`ssi-chat/src/chat_application/skills/`). Full routing specification: **[docs/intent-determination.md](docs/intent-determination.md)**.
