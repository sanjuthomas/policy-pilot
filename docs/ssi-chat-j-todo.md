...
**Success bar:** golden eval green against `http://localhost:8096` (**101** cases in `ssi-chat-j/eval/`). Python `ssi-chat` and `cypher-builder-svc` are **retired**.
...
**Bank snapshot:** prove bank **101** (policies 11 · me 18 · skills 17 · graph/entity/SoD/vector remainder).
...
prove bank contains **101** cases, including the later `me`, skill, facet, and SoD
...
| P4.4 | **Success bar met** | `done` | Current Java prove bank **101**; Python-only **0** |
...
| ID | Item | Status | Notes |
|----|------|--------|-------|
| D.1 | Harvest `ssi-chat/regression/questions.yaml` into Java goldens | `todo` | Local-only Python bank (gitignored). Do **not** add a Java `questions.yaml`. Promote uncovered cases into `eligibility_golden.yaml`. Done: `golden_instructions_conflicts_ficc`, `golden_payments_largest_who_created`. |
...
| 2026-07-25 | Governed audit evidence extended across create / submit / approve / cancel payment skills; prove bank **100** |
...
| 2026-07-26 | D.1 reframed: harvest strong uncovered cases from local `ssi-chat/regression/questions.yaml` into `eligibility_golden.yaml`; do not add a Java `questions.yaml` |
