...
Web UI and CLI helpers for end-to-end testing of the **instruction lifecycle** and **payment lifecycle** APIs with **ZITADEL OIDC** authentication.

Use it to generate realistic traffic that flows through Kafka into the ETL and chat pipeline, including OPA policy demo scenarios that verify `ALERT` and `INFO` security event counts.
...
| `SECURITY_EVENTS_COLLECTION` | `instruction-lifecycle-manager` |
