  ...
  summaryEl.innerHTML = `
    <div class="detail-card">
      ...
      <dl class="detail-grid">
        ...
        ${field("Resource status", resource.status)}
        ${field("Owning LOB", resource.owning_lob)}
        ${field("Instruction type", resource.instruction_type)}
        ${field("Resource version", resource.version_number)}
...
async function loadEvent() {
  ...
  document.title = `Security Event · ${eventId}`;
