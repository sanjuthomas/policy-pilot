"""Manages the instruction-service's own Zitadel session for authz OBO calls."""
...
class ServiceIdentity:
    """Holds the instruction-service session token for authorization-service OBO calls."""
    ...
    async def login(self, *, max_attempts: int = 5, retry_delay_s: float = 2.0) -> None:
        if not settings.zitadel_service_pat:
            logger.warning(
                "zitadel_service_pat not configured — authz OBO delegation will be unavailable"
            )
            return
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                logger.info(
                    "instruction-service authenticated as %s (session_id=%s)",
                    user_id,
                    self._session_id,
                )
                return
                ...
                if attempt < max_attempts:
                    logger.warning(
                        "instruction-service login attempt %s/%s for %s failed: %s — retrying",
                        attempt,
                        max_attempts,
                        user_id,
                        exc,
        ...
        logger.error(
            "instruction-service could not authenticate as %s after %s attempts: %s — "
            "authz OBO delegation will be unavailable",
            user_id,
            max_attempts,
            last_exc,
        )
